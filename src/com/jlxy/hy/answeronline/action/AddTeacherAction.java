...
public class AddTeacherAction extends FenYeAction {
	private List content; // 显示内容
	private String uid; // 用户ID

	private String uname; // 用户名
	private String pwda; // 教师密码
	...
	public String getSql() {
		sql = " select UID,UName,URole from DY_User where URole<2";
		return sql;
	}
	...
	public String execute() throws Exception {
		if ("del".equals(actionStr)) {
			...
			if (dbu.update(sql)) {
				message = "alert('成功删除用户')";
			} else {
				message = "alert('未知错误教师失败')";
			}
			this.setCurPage(1);
		}
		if ("add".equals(actionStr)) { // 添加教师
