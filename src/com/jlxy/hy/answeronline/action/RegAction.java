...
public class RegAction extends UnameUniqueAction {
	...
	public String execute() throws Exception {
		...
		if (!randNum.equals(valcode)) { // 如果验证码不相同
			message = "验证码输入错误，注册失败，请重新输入。现" + "在将要跳转到注册页面"; // 给出提示信息
			url = "reg.jsp";
			return SUCCESS;
		}
		if (this.getFlag()) { // 注册名已经存在
			message = "您所注册的用户名已经存在，请重新输入。现" + "在将要跳转到注册页面"; // 给出提示信息
			url = "reg.jsp";
			return SUCCESS;
		}
		...
		if (dbu.update(sql)) {
			message = "恭喜您，注册成功，欢迎来到金陵学院。" + "现在将要跳转到主页"; // 注册成功的提示消息
