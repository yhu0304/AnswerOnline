...
public class DBUtil{
	...
	private DataSourceTransactionManager dtm;				 //jdbc事务管理器
	private DefaultTransactionDefinition dtd;               //事务定义
	...
	public List getIndexContent(String sql){
		...
		try{
			sql = new String(sql.getBytes(),"gbk");		//对SQL进行转码
			...
			if(rl.size()!=0){									//结果不为空的情况
				for(int i=0;i<rl.size();i++){					//遍历结果
					...
					gi.setGid(new String(map.get("gid").toString().getBytes("gbk"),"gbk"));		//设置课程ID值
					...
					gi.setTopic(new String(map.get("topic").toString().getBytes("gbk"),"gbk"));	//设置问题数量属性值
					gi.setRevert(new String(map.get("revert").toString().getBytes("gbk"),"gbk"));	//设置回复数量属性值
					gi.setUname(new String(map.get("uname").toString().getBytes("gbk"),"gbk"));
					gi.setTid(new String((byte[])map.get("tid"),"gbk"));	//设置最后回复问题的ID值
	...
	public int getTotal(String sql,int span){						//得到总页数
		int total = 0;										//声明页数返回值
		try{
			sql = new String(sql.getBytes(),"gbk");	//对sql语句转码
		...
		try{
			...
			if(rl.size()!=0){						//结果不为空的情况
				for(int i=0;i<rl.size();i++){		//遍历结果
					...
					if (email != null) {
						user.setEmail(email.toString());//设置User对象的email属性
					}
					Object URegDate = map.get("URegDate");
					if (email != null) {
						user.setRegDate(URegDate.toString());//设置User对象的email属性
					}			
					user.setPermit(map.get("UPermit").toString());	
	...
	public String getStringInfo(String sql){
		...
		try{
			...
			info = new String(info.getBytes("gbk"),"gbk");//对查询的字符串转码
		}
		catch(Exception e){
	...
	public boolean isExist(String sql){
		...
		rl = jt.queryForList(sql);							//执行查询,返回一个list对象
        if(rl.size()!=0){									//判断查询结果,size()返回列表中的元素数
	...
	public boolean update(String sql){							//执行单个SQL语句的方法
		boolean flag = false;
		try{
			sql = new String(sql.getBytes(),"gbk");//对SQL语句进行转码
